...
void ResourceImporterPLY::get_import_options(const String &p_path, List<ImportOption> *r_options, int p_preset) const {
    ...
    // strict no-op (keep all / off) so Ultra imports stay byte-identical.
    // prune_ratio is the portable, scale-invariant control (keep this fraction
    // of splats by importance). prune_importance_threshold is an ADVANCED,
    // scale-DEPENDENT absolute cutoff (importance scales with the asset's
    // world-unit scale per the ADR), so it is safe to leave at 0.0 unless tuned
    // per-asset.
    r_options->push_back(ImportOption(
...
Error ResourceImporterPLY::import(ResourceUID::ID p_source_id, const String &p_source_file, const String &p_save_path,
    ...
    // (both read the asset's now-pruned gaussian data). No-op at the default
    // (1.0 / 0.0) -> saved_count == final_count, Ultra byte-identity preserved.
    ...
    // count that entered pruning (post density / max_splats) so an A/B can see the
    // prune-only reduction. When pruning is off, saved_count == final_count.
    ...
    // #349). Fetch it once here and reuse it for the saved bounds AABB so both
    // the bounds metadata and the baked chunks reflect the pruned arrays.
    ...
    GS_LOG_STREAMING_INFO(vformat("PLY import successful: %d/%d splats (pre-prune %d) from %s",
