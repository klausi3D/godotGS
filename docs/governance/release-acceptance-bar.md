...
> the other 37 are neither closed nor ledgered. *(Amended 2026-09-25: a sixth entry, **#54**,
> was added after its acceptance. #54 is `priority:P2`, so it is not one of the 38 and moves
> neither count.)*
...
what moves the count from 8/4 to 9/3; the three then still open were #851, #833 and #54.) **Plus four
...
5. ~~**#929** — splats swim under FSR2~~ — the **static-camera** jitter defect, **fixed**
   ...
   **What happened.** The jitter fix merged as #1026 saying `Refs #929`, never a
   ...
   above was re-counted to 9 fixed or closed / 3 open (since re-counted again for #54).
...
10. ~~**#54** — dropped tiles when overlap-record demand outruns the allocated capacity~~
   ...
   disclosed public-alpha limitation**
   ([#54 comment 5838456717](https://github.com/klausi3D/godotGS/issues/54#issuecomment-5838456717)).
   #54 stays open as the tracking issue for the engineering fix. The entry is kept struck
   rather than deleted so the disposition it was waiting on stays readable. **Reproduced and characterised on hardware 2026-09-22** (see the §8.1
   ...
   render — a capacity-envelope defect, not a correctness one. The disclosure that
   was proposed here was accepted, so its three consequences have been carried out:
   the §8.1 `#54` row has lost its *proposed* marker, the user-facing entry has moved
   out of the known-limitations page's fenced "Proposed, not yet accepted" section
   into its accepted part, and this item is struck with the status line above
   re-counted. #54 also has a `public_alpha_issue_ledger` entry, so the gate resolves it
   as `accepted_alpha_limitation` whenever a candidate's issue snapshot contains it
   (§9.1). **The one thing that would undo
   this is a measurement of a default-configured in-envelope scene exceeding the 100M
   cap**: the acceptance lapses on it, and #54 returns to this list. The extrapolated
