...
- **Import preset and node quality pinned, not defaulted.** The default
  import preset is Ultra (`max_splats = 0`, `density_multiplier = 1.0`,
  `io/gaussian_import_preset.cpp:10-19`), which keeps every splat. The renderer then
  budgets for the asset's whole count rather than the node's `max_splat_count`
  (`renderer/gaussian_splat_renderer.cpp:127-134`, `:1617-1619`). A lighter preset
  thins the scene: `desktop` is `max_splats = 750000`, `density_multiplier = 0.7`
  (`:56-57` of the preset file). On such an asset the node's Balanced preset also caps it
  at 500,000. (An earlier revision of this bullet called `desktop` the default and
  said a pass at defaults signs off a scene thinned fourfold. The Ultra default
  predates this document, so that was never true.) Two passes at "defaults" can
  therefore render very different splat counts. The pass **must** state
...
10. **#54** — dropped tiles when overlap-record demand outruns the allocated capacity
   ...
   default-configured in-envelope scene exceeding the 100M cap. At default import
   settings no splat limit bounds the visible set, and the extrapolated cap is reached
   at ≈1.8M visible splats at 1080p. That is an estimate, and whether real scenes
