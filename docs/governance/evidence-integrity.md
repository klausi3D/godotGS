...
| --- | --- | --- |
| **Fail-open on absence** | An empty, missing or unreadable input yields the *most permissive* result. | `classify_change.py` returned `R0` — the lowest risk class — for an empty changed-path set, while `policy.json` declares `default_unclassified: R3` (#812). The row was written before the code was changed: at the time it read as past tense, the defect was still live and its unit test asserted the `R0` value, so the suite defended it (`GS-AUDIT-TEST-002`). It now falls back to `default_unclassified`, and an unresolvable base ref is an error rather than a degraded diff. A skipped guard emitted `passed: true` with a `fail_reason` nobody reads (#811). |
