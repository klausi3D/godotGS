...
| `base_sha_proven_failing` | yes | The base commit SHA on which the failure was reproduced (charter §2.7). |
...
- **Quarantined lane that fails (or has no executed coverage):** tolerated but
  loudly reported and counted, not silently swallowed:
...
- **Schema guard:** runs in the fast `--guard-only` lane. It fails on a missing
  required field, an unknown `lane`, or a past `expires_utc`. An empty manifest
  passes trivially.

## Adding an entry (Slice 2 - human-gated)

Every quarantine entry is an individually human-approved decision (charter §6).
