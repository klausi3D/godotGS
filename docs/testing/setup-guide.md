...
- Active batches: `CompositorHazard`, `OutputCompositor`, `TileRenderer`, `Integration`, `RendererPipeline`, `Lifetime`, and the `[SceneTree]` batches (`NodeSceneTree`, `WorldSceneTree`, `SceneDirectorSceneTree`, `RendererSceneTree`) — the `[SceneTree]` corpus (#329) now runs in these dedicated batches rather than being excluded. Still catalogued-but-empty: `ComputeInfrastructure`, `GpuSorting`, `MemoryStream`, `Streaming`.
