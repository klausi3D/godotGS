...
A matching count is necessary but **not sufficient**. A count, an encoding and a property block all
describe a file *shape*, and a shape can be assembled or copied from another fixture — sphere,
cube, plane and torus are all 50000-splat C++ outputs with identical headers. So the label comes
from the **producer record**: `prepare_synthetic_assets.py` writes
`tests/fixtures/.fixture_provenance.json` as it generates, recording which producer wrote each
file, keyed by the file's SHA-256 and bound to the filename it was written as. The consumer
project's byte-identical copy authenticates from the same entry. The record is gitignored and
regenerated with the fixtures; it is never written by hand.

| | effect |
| --- | --- |
| Count matches a declared producer **and** the record names these bytes under this filename as that producer (a `cpp_rich` label also needs the rich SH header block) | the lane is labelled `python_fallback` or `cpp_rich` |
| Count matches a declared producer but the record is missing, unreadable, a different version, or does not name these bytes under this name | preflight fails `UNRECOGNIZED`, and the message says which evidence was missing |
...
The `UNRECOGNIZED` rows fail **preflight itself** — whether or not `--require-asset-variant` is
passed — because a fixture no producer can be shown to have written cannot be attributed to any
workload. A fixture that is exactly the right size but was copied in, carried over from a checkout
that predates the record, or regenerated by hand lands there. The fix is always the same: run
`python tests/runtime/prepare_synthetic_assets.py --godot-binary ./bin/<godot built with tests=yes>`,
which rewrites both the fixtures and the record.
