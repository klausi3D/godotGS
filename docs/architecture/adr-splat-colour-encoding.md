...
**In scope, as part of slice 2** (review round 3; formerly slice 5): `GS-AUDIT-PERS-016`. It consumes this contract, and the helpers
...
after slices 3 and 4. In that window the brush blends a display-space target straight into
`sh_dc` (`core/gaussian_data_edits.cpp:317-322`) and the grading bake reads and writes `sh_dc`
as a display colour (`core/gaussian_data_color_grading.cpp:34-43`). The shader then adds 0.5
again, and saving persists the corrupted value. The consumers need slice 2's display helpers,
