...
`MODULE_TEST_FILTERS` declares 28 lanes: **22 strict, 6 advisory** (`GaussianSplatting
...
> **And in part by #586.** On the same precedent, #586 promoted `[SortFallback]` — the
> policy that decides whether a frame is presented with incorrect alpha compositing or
> rejected — out of the advisory `[untagged]` net into its own strict lane, taking *Context*
> from 27 lanes / 21 strict to **28 lanes / 22 strict**. #846 and #586 were developed in
> parallel and merged through a conflict resolution that took the **union** of both
> promotions; the counts above are the merged reality, and are derived from
> `MODULE_TEST_FILTERS` rather than hand-tallied (see the one-liner in
> [Build / test / CI reference](../reference/build-test-ci.md#per-lane-result-ledger)).
> The advisory set is still 6, unchanged.
