...
const MEMORY_STREAM_FIELDS := [
	...
	# `memory_stream_defrag_count` used to be here. No monitor definition
	# registers it, so `_m()` returned its 0.0 fallback and the row showed a
	# permanent, silent 0 that read exactly like a measurement. Removed with
	# its label nodes rather than left lying. Refs #833.
]
...
# Call sites pass the FULL id, spelled out. tests/ci/check_shipped_project_scripts.py
# checks every monitor id a shipped script reads against the ids
# performance_monitors.cpp registers, and derives both sides from string
# literals; building the id from a prefix at run time hid every read here from
# that check. That blind spot is how `memory_stream_defrag_count` -- registered
# nowhere -- sat in this panel as a permanent, silent 0.
func _m(id: String) -> float:
	if Performance.has_custom_monitor(id):
		return Performance.get_custom_monitor(id)
	return 0.0

func _has_monitor(id: String) -> bool:
	return Performance.has_custom_monitor(id)
...
func _is_streaming_active() -> bool:
	if _has_monitor("gaussian_splatting/streaming_monitor_ready") and int(_m("gaussian_splatting/streaming_monitor_ready")) == 1:
		return true
	return _m("gaussian_splatting/streaming_total_chunks") > 0.0 \
		or _m("gaussian_splatting/streaming_loaded_chunks") > 0.0 \
		or _m("gaussian_splatting/streaming_visible_chunks") > 0.0
...
func _refresh() -> void:
	...
	# GPU Timing
	var frame = _m("gaussian_splatting/gpu_time_frame_ms")
	var cull = _m("gaussian_splatting/gpu_time_cull_ms")
	var bin = _m("gaussian_splatting/gpu_time_binning_ms")
	var prefix = _m("gaussian_splatting/gpu_time_prefix_ms")
	var raster = _m("gaussian_splatting/gpu_time_raster_ms")
	var resolve = _m("gaussian_splatting/gpu_time_resolve_ms")
	var cpu_setup = _m("gaussian_splatting/cpu_setup_time_ms")
	...
	# Visibility
	_set_label("visible_splats", _n(_m("gaussian_splatting/visible_splats")))
	_set_label("total_processed", _n(_m("gaussian_splatting/total_processed")))
	_set_label("projection_success_count", _n(_m("gaussian_splatting/projection_success_count")))
	var success_pct = _m("gaussian_splatting/projection_success_rate_pct")
	_set_label("projection_success_rate_pct", "%.1f%%" % success_pct, _pct_color(success_pct))

	# Rejections
	_set_label("clip_reject_count", _n(_m("gaussian_splatting/clip_reject_count")))
	_set_label("radius_reject_count", _n(_m("gaussian_splatting/radius_reject_count")))
	_set_label("viewport_reject_count", _n(_m("gaussian_splatting/viewport_reject_count")))
	_set_label("extreme_aspect_count", _n(_m("gaussian_splatting/extreme_aspect_count")))
	_set_label("index_mismatch_count", _n(_m("gaussian_splatting/index_mismatch_count")))

	# Tiles
	_set_label("tile_count", _n(_m("gaussian_splatting/tile_count")))
	_set_label("overflow_tile_count", _n(_m("gaussian_splatting/overflow_tile_count")))
	_set_label("clamped_records", _n(_m("gaussian_splatting/clamped_records")))
	_set_label("aggregated_count", _n(_m("gaussian_splatting/aggregated_count")))

	var streaming_active = _is_streaming_active()

	# VRAM
	if streaming_active:
		_set_label("vram_current_usage_mb", "%.1f MB" % _m("gaussian_splatting/vram_current_usage_mb"))
		_set_label("vram_budget_mb", "%.1f MB" % _m("gaussian_splatting/vram_budget_mb"))
		var vram_pct = _m("gaussian_splatting/vram_usage_percent")
		_set_label("vram_usage_percent", "%.1f%%" % vram_pct, _pct_color(100.0 - vram_pct, 50.0))
		_set_label("vram_current_max_chunks", _n(_m("gaussian_splatting/vram_current_max_chunks")))
		_set_label("vram_loaded_chunks", _n(_m("gaussian_splatting/vram_loaded_chunks")))
		_set_label("vram_evicted_this_frame", _n(_m("gaussian_splatting/vram_evicted_this_frame")))
		_set_label("vram_loaded_this_frame", _n(_m("gaussian_splatting/vram_loaded_this_frame")))
		_set_label("vram_budget_warning_active", "%d" % int(_m("gaussian_splatting/vram_budget_warning_active")))
		_set_label("vram_regulation_adjustments", _n(_m("gaussian_splatting/vram_regulation_adjustments")))
		_set_label("vram_thrashing_events", _n(_m("gaussian_splatting/vram_thrashing_events")))
	else:
		_set_group_na(VRAM_FIELDS)

	# Memory Stream
	if streaming_active:
		_set_label("memory_stream_total_bytes_uploaded_mb", "%.2f MB" % _m("gaussian_splatting/memory_stream_total_bytes_uploaded_mb"))
		_set_label("memory_stream_total_bytes_downloaded_mb", "%.2f MB" % _m("gaussian_splatting/memory_stream_total_bytes_downloaded_mb"))
		_set_label("memory_stream_buffer_switches", _n(_m("gaussian_splatting/memory_stream_buffer_switches")))
		_set_label("memory_stream_stalls", _n(_m("gaussian_splatting/memory_stream_stalls")))
		var stall_pct = _m("gaussian_splatting/memory_stream_stall_percent")
		_set_label("memory_stream_stall_percent", "%.1f%%" % stall_pct, _pct_color(100.0 - stall_pct, 95.0))
		_set_label("memory_stream_pool_hits", _n(_m("gaussian_splatting/memory_stream_pool_hits")))
		_set_label("memory_stream_pool_misses", _n(_m("gaussian_splatting/memory_stream_pool_misses")))
		var pool_hit = _m("gaussian_splatting/memory_stream_pool_hit_rate_pct")
		_set_label("memory_stream_pool_hit_rate_pct", "%.1f%%" % pool_hit, _pct_color(pool_hit))
		_set_label("memory_stream_peak_memory_mb", "%.1f MB" % _m("gaussian_splatting/memory_stream_peak_memory_mb"))
	else:
		_set_group_na(MEMORY_STREAM_FIELDS)

	# Streaming
	if streaming_active:
		_set_label("streaming_total_chunks", _n(_m("gaussian_splatting/streaming_total_chunks")))
		_set_label("streaming_visible_chunks", _n(_m("gaussian_splatting/streaming_visible_chunks")))
		_set_label("streaming_loaded_chunks", _n(_m("gaussian_splatting/streaming_loaded_chunks")))
		_set_label("streaming_frustum_culled_chunks", _n(_m("gaussian_splatting/streaming_frustum_culled_chunks")))
		_set_label("streaming_vram_usage_mb", "%.1f MB" % _m("gaussian_splatting/streaming_vram_usage_mb"))
		_set_label("streaming_chunks_loaded_this_frame", _n(_m("gaussian_splatting/streaming_chunks_loaded_this_frame")))
		_set_label("streaming_chunks_evicted_this_frame", _n(_m("gaussian_splatting/streaming_chunks_evicted_this_frame")))
		_set_label("streaming_visible_count", _n(_m("gaussian_splatting/streaming_visible_count")))
		_set_label("streaming_buffer_capacity_splats", _n(_m("gaussian_splatting/streaming_buffer_capacity_splats")))
		_set_label("streaming_effective_splat_count", _n(_m("gaussian_splatting/streaming_effective_splat_count")))
		_set_label("streaming_visible_change_ratio", "%.2f" % _m("gaussian_splatting/streaming_visible_change_ratio"))
		_set_label("streaming_lod_blend_factor", "%.2f" % _m("gaussian_splatting/streaming_lod_blend_factor"))
		_set_label("streaming_sh_band_level", "%d" % int(_m("gaussian_splatting/streaming_sh_band_level")))
		_set_label("streaming_bytes_uploaded_mb", "%.2f MB" % _m("gaussian_splatting/streaming_bytes_uploaded_mb"))
		_set_label("streaming_buffer_switches", _n(_m("gaussian_splatting/streaming_buffer_switches")))
	else:
		_set_group_na(STREAMING_FIELDS)

	# LOD
	if streaming_active:
		_set_label("lod_current_level", "%d" % int(_m("gaussian_splatting/lod_current_level")))
		_set_label("lod_distance_multiplier", "%.2f" % _m("gaussian_splatting/lod_distance_multiplier"))
		_set_label("lod_target_distance", "%.1f" % _m("gaussian_splatting/lod_target_distance"))
		_set_label("lod_hysteresis_zone", "%.2f" % _m("gaussian_splatting/lod_hysteresis_zone"))
		_set_label("lod_blend_distance", "%.1f" % _m("gaussian_splatting/lod_blend_distance"))
		_set_label("lod_transitions_this_frame", "%d" % int(_m("gaussian_splatting/lod_transitions_this_frame")))
		_set_label("lod_splat_skip_factor", "%.2f" % _m("gaussian_splatting/lod_splat_skip_factor"))
		_set_label("lod_opacity_multiplier", "%.2f" % _m("gaussian_splatting/lod_opacity_multiplier"))
		_set_label("lod_effective_count_after_skip", _n(_m("gaussian_splatting/lod_effective_count_after_skip")))
		_set_label("lod_chunk_blend_factors_avg", "%.2f" % _m("gaussian_splatting/lod_chunk_blend_factors_avg"))
		_set_label("lod_chunks_in_transition", "%d" % int(_m("gaussian_splatting/lod_chunks_in_transition")))
		_set_label("lod_quality_degradation_active", "%d" % int(_m("gaussian_splatting/lod_quality_degradation_active")))
		_set_label("lod_min_chunk_distance", "%.1f" % _m("gaussian_splatting/lod_min_chunk_distance"))
		_set_label("lod_max_chunk_distance", "%.1f" % _m("gaussian_splatting/lod_max_chunk_distance"))
		_set_label("lod_avg_chunk_distance", "%.1f" % _m("gaussian_splatting/lod_avg_chunk_distance"))
		var reduction = _m("gaussian_splatting/lod_reduction_ratio_pct")
		_set_label("lod_reduction_ratio_pct", "%.1f%%" % reduction)
		_set_label("lod_level_0_chunk_count", "%d" % int(_m("gaussian_splatting/lod_level_0_chunk_count")))
		_set_label("lod_sh_band_3_chunk_count", "%d" % int(_m("gaussian_splatting/lod_sh_band_3_chunk_count")))
	else:
		_set_group_na(LOD_FIELDS)

	# Prefetch
	if streaming_active:
		_set_label("chunk_prefetch_hits", _n(_m("gaussian_splatting/chunk_prefetch_hits")))
		_set_label("chunk_prefetch_misses", _n(_m("gaussian_splatting/chunk_prefetch_misses")))
		var prefetch_eff = _m("gaussian_splatting/chunk_prefetch_efficiency_pct")
		_set_label("chunk_prefetch_efficiency_pct", "%.1f%%" % prefetch_eff, _pct_color(prefetch_eff))
		_set_label("chunk_camera_velocity", "%.2f" % _m("gaussian_splatting/chunk_camera_velocity"))
		_set_label("chunk_average_load_time_ms", "%.2f ms" % _m("gaussian_splatting/chunk_average_load_time_ms"))
		_set_label("chunk_upload_queue_depth", "%d" % int(_m("gaussian_splatting/chunk_upload_queue_depth")))
		_set_label("chunk_pack_jobs_in_flight", "%d" % int(_m("gaussian_splatting/chunk_pack_jobs_in_flight")))
		_set_label("chunk_total_capacity_mb", "%.1f MB" % _m("gaussian_splatting/chunk_total_capacity_mb"))
	else:
		_set_group_na(PREFETCH_FIELDS)

	# Pack Timing
	if streaming_active:
		var pack_avg = _m("gaussian_splatting/pack_avg_time_ms")
		_set_label("pack_avg_time_ms", "%.2f ms" % pack_avg, _time_color(pack_avg))
		var pack_max = _m("gaussian_splatting/pack_max_time_ms")
		_set_label("pack_max_time_ms", "%.2f ms" % pack_max, _time_color(pack_max))
		_set_label("pack_jobs_completed", "%d" % int(_m("gaussian_splatting/pack_jobs_completed")))
		_set_label("upload_mb_this_frame", "%.2f MB" % _m("gaussian_splatting/upload_mb_this_frame"))
		_set_label("upload_chunks_this_frame", "%d" % int(_m("gaussian_splatting/upload_chunks_this_frame")))
	else:
		_set_group_na(PACK_FIELDS)

	# SH Cache
	_set_label("sh_cache_hits", _n(_m("gaussian_splatting/sh_cache_hits")))
	_set_label("sh_cache_updates", _n(_m("gaussian_splatting/sh_cache_updates")))
	var sh_hit = _m("gaussian_splatting/sh_cache_hit_rate_pct")
	_set_label("sh_cache_hit_rate_pct", "%.1f%%" % sh_hit, _pct_color(sh_hit))
	if streaming_active:
		_set_label("sh_compression_raw_mb", "%.2f MB" % _m("gaussian_splatting/sh_compression_raw_mb"))
		_set_label("sh_compression_compressed_mb", "%.2f MB" % _m("gaussian_splatting/sh_compression_compressed_mb"))
		var sh_ratio = _m("gaussian_splatting/sh_compression_ratio_pct")
